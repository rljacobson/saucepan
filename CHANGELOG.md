...
* extracted from the original `codespan` repo
* `File` is now `pub`lic
* Renamed `File` —> `Source` and `Files` —> `Sources` and related names, as source code need not come from a file.
* `Span` and `Source` (prev. `File`) now optionally know their `SourceID`.
* `SourceID` (prev. `FileId`) is generic in `Sources`.
